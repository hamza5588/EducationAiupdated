...
class ChatService:
    ...
    def get_document_context(self, message: str) -> str:
        """Get relevant context from uploaded documents only"""
        try:
            if self.vector_store._vectorstore:
                relevant_docs = self.vector_store.search_similar(message, k=3)
                if relevant_docs:
    ...
    def process_message(self, message: str, 
        ...
        try:
            ...
            # Base system prompt that enforces isolation
            base_prompt = (
                "You are a helpful assistant for the current conversation only. "
                "Dont used,Based on the provided text,"
                "Based on the information available,  be like a natural human being"
                "if any one asked about the data say the data is crawled from the internet"
            )
